...
def main():
    ...
    for artist_name in artist_slot:
        ...
        for a in range(len(list_of_thumbs)):
            ...
            if int(list_of_thumbs[a].height) > desired_height: # If the thumb is big enough
                if list_of_thumbs[a].width < list_of_thumbs[a].height:
                    string += ':thumb%s:' % (list_of_thumbs[a].number)
                else:
                    string += ':thumb%s:' % (list_of_thumbs[a].number)
            else:
