...
function openColorTools(input_box){
    input_box.before('<div class="color_tools"><div class="square teal"></div><div class="square pink"></div><div class="square yellow"></div></div>');
...
/** Gets data from local storage 
    ...
    // If the user has no info to begin with (new user)
        ...
        addGrid();
        addGrid();
        $('#title0').val('Getting Started');
        $('#title1').val('Click to add a title');

        grid.add_widget('<li><div class="text">Welcome to <b>Things I Wanna Do.</b></div><div class="edit"></div></li>', 1, 4, 1, 4);
        grid.add_widget('<li><div class="text">I\'m a card. Try dragging me around.</div><div class="edit"></div></li>', 1, 4, 1, 8);
        grid.add_widget('<li><div class="text">Double click me or press the pencil icon in the right hand corner to edit.</div><div class="edit"></div></li>', 1, 5, 1, 12);
        grid.add_widget('<li><div class="text">Press Enter or click anywhere off the card to save your card.</div><div class="edit"></div></li>', 1, 4, 1, 17);
        grid.add_widget('<li><div class="text">Clear or delete this grid by clicking the gear icon above the title.</div><div class="edit"></div></li>', 1, 5, 1, 21);
...
function backTitle(){
    $.each($('.title_input'), function() {
        ...
        addTitleColor($(this), 'pink');
        addTitleColor($(this), 'yellow');
        //$(this).parent().attr('data-sizey', 3)
    });
}

function addTitleColor(el, color){
    if (el.hasClass(color)){
        el.parent().addClass(color);
    }
}
...
function createTitles(index){
    ...
    color_index = 0;
    grid.add_widget('<li><input type="text" class="title_input ' + colors[color_index] + '" id="title' + index + '" name="title' + index + '" value="Title" onclick="this.select()"><div class="plus">+</div></li>', 1, 3, index + 1, 5);
