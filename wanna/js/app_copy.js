...
function prepareTitles(num, add){
/** Prepares the bars */
    if (add === undefined){
        add = 0;
    }
    num = parseInt(num);
    $('#title').append('<div id="title_wrap' + (num - add) + '" class="title_wrap"><div class="settings"></div><div class="tooltip"><div class="clear_button">Clear Grid</div><div id="del_title' + (num - add) + '" class="del_title">Delete Grid</div></div><div class="progress"><div id="bar' + (num + 1) + '" class="bar"></div></div></div>');
}

function prepareGrids(){
    $('#title').append('<div class="title_wrap"><div class="add_grid" onclick="addGrid(1)"><span>Add Grid</span>+</div></div>');
...
/* Sets up so when a sqaure color is clicked, the widget changes color */
...
/* Strips all possible color classes off the title widget */
    for (i = 0; i < colors.length; i++){
...
/** Adds a new title and grid space 
    ...
    storeData(json);
    getData();
    var col = $('#ul .title_widget:last').attr('data-col');
    grid.add_widget('<li><div id="textarea"></div></li>', 1, 6, col, 100);
    editWidget($('#textarea'), 1);
}
...
/** Gets data from local storage 
    ...
    // If the user has no info to begin with (new user)
        ...
        grid.add_widget('<li><div class="text">Welcome to <b>wanna</b>.</div><div class="edit"></div></li>', 1, 3, 1, 4);
...
/** Adds the col number to the id, so starts counting at 1 */
    $.each($('.title_input'), function() {
...
/** Creates one title with the given index, color, and input value */
    color = color;
    if (color === undefined){
        color = 'teal';
    }
...
// change back functions to seraialize function