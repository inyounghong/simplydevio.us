<?
session_start();
header('Content-type: image/png');
$visitor = exec("python ../image_with_name.py 't-raposo'");
$new_message1 = str_replace("visitor",$visitor, 'Hi,');
$new_message2 = str_replace("visitor",$visitor, 'Welcome to my page!');
$font = './uploaded_fonts/' . 'ComingSoon.ttf';
# Determine the longer of the two  messages
if (strlen($new_message2) > 0)
{
    if (strlen($new_message1) <= strlen($new_message2))
    {
        $longer_message = $new_message2;
    }
    else
    {
        $longer_message = $new_message1;
    }
}
else
{
    $longer_message = $new_message1;
}
$width  = (23 * strlen($longer_message));
$im = imagecreatetruecolor ($width,148);
$background = imagecolorallocate($im, 224, 224, 224);
imagefill($im,0,0,$background);
$color = imagecolorallocate($im, 68, 68, 68);
imagettftext($im, 24, 0, 117, 56, $color, $font, $new_message1);
imagettftext($im, 24, 0, 70, 101, $color, $font, $new_message2);
imagepng($im);
imagedestroy($im);
?>